    ...
    fn exec(vm: &mut Vm) -> Result<(), RuntimeError> {
        ...
        if let Some(class_ptr) = operand1.class_ptr() {
            if vm.scope_stack.has_impl(class_ptr, add_trait) {
                ...
                Err(RuntimeError::new("Equals trait not implemented"))
            }
        } else {
            Err(RuntimeError::new("Equals trait not implemented"))
        }
